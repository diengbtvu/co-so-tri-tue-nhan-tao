...
namespace BFS
{
    class Dothi
    {
        ...
        public Dothi()
        {
            this.sodinh = -1;
            this.start = -1;
            this.matran = new int[7, 7]; // Có thể sử dụng 1 hằng nào đó nếu bạn muốn
            readDothi();                // Gọi phương thức đọc file .txt
        }

        public void readDothi() // Đọc file đồ thị
        {
            string textFile = @"c:\users\nguyen hoang\documents\visual studio 2013\Projects\co-so-tri-tue-nhan-tao\BFS\BFSinput.txt"; // Thay đổi đường dẫn phù hợp với ứng dụng của bạn
